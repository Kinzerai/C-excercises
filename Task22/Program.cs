...
{
    double XYZ = Math.Round(Math.Sqrt(Math.Pow(x,2)+ Math.Pow(y,2)+ Math.Pow(z,2)),2);
    return XYZ;
}
...
for (int i=0; i<2; i++)
{
    ...
    Console.Write($"Введите координату Х точка №{i+1}: ");
    x =Convert.ToInt32(Console.ReadLine()) - x;
    Console.Write($"Введите координату Y точка №{i+1}: ");
    y =Convert.ToInt32(Console.ReadLine()) - y;

    if (count == 3)
    {
        Console.Write($"Введите координату Z точка №{i+1}: ");
